...
prx.types.ios7_switch = {
	...
	,onDisplay: function(item,containerid,symbol) {
		...
		cR += '#'+_id+' input:checked + label .switch { left: '+(_dims.width - _dims.height + parseInt(prx.componentsHelper.getProp(item.borderWidth,'num-border-width')))+'px; '+((item.name=='ios10_switch') ? '' : 'border-color: '+prx.componentsHelper.getProp(item.activeBorderColor,'color-border')+';')+' }';
		cR += '</style>'
		cR += prx.items.getComponentPrependDivs(item, containerid, symbol);
...
prx.types.ios7_alert = {
	...
	,onDisplay: function(item,containerid,symbol) {
		...
		var cR = '';
		cR += '<div id="' + _id + '" ' + prx.items.getComponentBaseAttributes(item, containerid, symbol)  + ' class="' + prx.items.getComponentBaseClasses(item, containerid, symbol) + ' box pos type-ios7-alert">';
...
prx.types.ios10_widget = {
	...
	interactions: [
		prx.commonproperties.actions,
		{
			caption: 'Secondary title text Interactions',
			name: 'actions1',
			type: 'action',
			value: function(item,name) {
				if(typeof(item.actions1) == "undefined") {
